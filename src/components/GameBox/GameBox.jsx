...
const GameBox = ({type}) => {
  ...
  useEffect(() => {
    ...
    .then(result => {
      ...
      setGame(result.payload ? result.payload : false)
    })
    .catch(error => setErrorDialog({ show: true, message: error.message }))
  }, [type])

  if (game === null) return <Box sx={loaderBox}><CircularProgress size='80px' />{ errorDialog.show ? <ErrorDialog text={errorDialog.message} closeFunc={setErrorDialog} /> : null }</Box>

  return (
    <Box sx={mainBox}>
      <Box sx={titleBox}>
        <Box>{ type === 'next' ? 'СЛЕДВАЩ МАЧ' : 'ПОСЛЕДЕН МАЧ' }</Box>
        { game ? <Box sx={dateTextStyle}>{formatDate(game.date)}</Box> : null}
      </Box>
      {
        game === false
          ? <Box textAlign='center' mt={5}>Липсва информация</Box>
